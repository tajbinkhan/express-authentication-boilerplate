...
// src/seed/seeders/seedUser.ts
var seedUser_exports = {};
__export(seedUser_exports, {
  default: () => seedUser_default
});
module.exports = __toCommonJS(seedUser_exports);
...
// src/seed/seeders/seedUser.ts
var generateFakeUsers = (count) => {
  return Array.from({ length: count }, () => ({
    name: import_faker.faker.person.fullName(),
    username: import_faker.faker.internet.username(),
    email: import_faker.faker.internet.email(),
    image: import_faker.faker.image.avatar(),
    role: import_faker.faker.helpers.arrayElement(ROLE_LIST.enumValues)
  }));
};
var seedUsers = async () => {
  try {
    const seedData = generateFakeUsers(100);
    const hashedPassword = await (0, import_bcrypt.hash)("Bang@123", 10);
    ...
    await connection_default.insert(users).values(userData);
  } catch (error) {
    console.error("Error seeding users:", error);
  }
};
var seedUser_default = seedUsers;
//# sourceMappingURL=seedUser.cjs.map
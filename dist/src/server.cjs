...
// src/server.ts
var import_package = require("express/package.json");
...
// src/passport/passportGoogle.ts
var import_passport2 = __toESM(require("passport"), 1);
var import_passport_google_oauth20 = require("passport-google-oauth20");
var authenticationService2 = new AuthenticationService();
import_passport2.default.serializeUser(async (user, done) => {
  try {
    done(null, user.id);
  } catch (error) {
    done(error, false);
...
import_passport2.default.deserializeUser(async (id, done) => {
  try {
    const user = await authenticationService2.findUserById(id);
    done(null, user.data);
  } catch (error) {
    done(error, false);
  }
});
var passportGoogle_default = import_passport2.default.use(
  new import_passport_google_oauth20.Strategy(
    ...
    async (accessToken, _, profile, done) => {
      try {
        const user = await authenticationService2.createUserFromGoogle(profile, accessToken);
        done(null, user.data);
      } catch (error) {
...
var templatesPath = import_path2.default.join(__dirname, "public/templates");
var sendEmail = async ({
  email,
  emailSubject,
  template,
  ...
  const html = await import_ejs.default.renderFile(import_path2.default.join(templatesPath, `${template}.ejs`), data, {
    async: true
  });
  const mailOptions = {
    from: emailFrom,
...
var jwtTokenName2 = process.env.JWT_COOKIE_NAME;
var sessionCookieName = process.env.SESSION_COOKIE_NAME;
var authenticationMiddleware = async (req, res, next) => {
  const apiResponse = new ApiResponse(res);
  try {
    const token = req.cookies[jwtTokenName2];
    if (!token) {
      res.clearCookie(jwtTokenName2);
...
var authenticationRouter = (() => {
  ...
  router.get("/login/google", import_passport3.default.authenticate("google", { scope: ["profile", "email"] }));
  router.get(
    "/google/callback",
    import_passport3.default.authenticate("google", { failureRedirect: "/login" }),
    (req, res) => {
...
var convertToWebP = async (filePath, outputDir, fileName) => {
  const outputFilePath = import_path3.default.join(outputDir, `${fileName}.webp`);
...
function appRouter(app2, initialRoute = "") {
  const allRoutes = [];
  for (const { path: path6, router } of routes) {
    const basePath = `${initialRoute}${path6}`;
    app2.use(basePath, router);
...
var DrizzleSessionStore = class extends import_express_session.Store {
  async get(id, callback) {
    try {
      const result = await connection_default.select().from(sessions).where((0, import_drizzle_orm7.eq)(sessions.sessionId, id)).execute();
  ...
  async destroy(id, callback) {
    try {
      await connection_default.delete(sessions).where((0, import_drizzle_orm7.eq)(sessions.sessionId, id)).execute();
      callback?.();
    } catch (err) {
      callback?.(err);
    }
  ...
  async touch(id, session2, callback) {
    try {
      ...
      await connection_default.update(sessions).set({
        expires: expire
      }).where((0, import_drizzle_orm7.eq)(sessions.sessionId, id)).execute();
      callback?.();
    } catch (err) {
      callback?.();
    }
...
function serverErrorHandler(app2) {
...
app.use(`/${uploadDir}`, import_express6.default.static(import_path5.default.join(process.cwd(), "uploads")));
appLogger(app);
appRateLimiter(app);
app.set("trust proxy", 1);
app.use(session_default);
...
var emailEnvSchema = import_zod6.z.object({
...
app_default.listen(PORT, () => {
  const ipAddress = import_ip.default.address();
  console.log(import_picocolors2.default.magenta(`
\u25B2 Express.js ${import_package.version}`));
